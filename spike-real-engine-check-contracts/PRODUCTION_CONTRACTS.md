...
- External Resources with valid `res://` file paths use `$ref`; built-in subresources with `::` paths encode inline. Inline resources use exactly one of `class` or `script`, plus storage properties excluding `script`/`resource_path`.
...
- Depth limit 32 prevents infinite recursion, including cycles; entry limit 100000 is **per container**, not a global traversal budget.
- `decode` supports these representations for **trusted, grammar-validated** specs; it is not a comprehensive untrusted-input validator. Resource construction can execute scripts. Unknown/unsupported tags emit engine errors.
- Godot JSON parses every number as float; decode normalizes integral safe-range numbers to int. Thus plain JSON does not preserve the distinction between `42` and `42.0` as Godot scalar types. Nonfinite tags are supported by the GDScript codec; the Rust scaffold's test list mentions rejecting nonfinite input, which still requires coordination when that decoder is implemented.
- Rust `gdview::variant` is still a scaffold. These are real-engine codec round trips, **not** a claim of verified Rust/GDScript cross-decoder compatibility. `protocol_golden.json` freezes the full 47-case envelope from the verified `production-20260924-190908` run. The Python driver writes each new envelope to `protocol_actual.json` in its artifact directory and compares canonical JSON against the frozen golden; normal tests never regenerate it. This golden test needs no `gdview::variant` implementation.
