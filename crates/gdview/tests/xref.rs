...
fn summary(findings: &[Finding]) -> Vec<(FindingKind, String, String)> {
    findings.iter().map(|f| (f.kind, format!("{}:{}", f.at.path, f.at.line), f.target.clone())).collect()
}

fn scene(root_type: &str, script: Option<&str>, body: &str) -> String {
    let mut text = String::from("[gd_scene format=3]\n\n");
    if let Some(script) = script {
        text.push_str(&format!("[ext_resource type=\"Script\" path=\"{script}\" id=\"1_s\"]\n\n"));
    }
...
"#,
    ...
    let found = findings(&[
        ...
        ("player.gd", "extends \"res://base.gd\"\n\nfunc _on_timer_timeout():\n\tpass\n"),
        ("main.tscn", &main),
    ]);
    assert_eq!(
        summary(&found),
        [
            (FindingKind::MissingMethod, "res://main.tscn:12".into(), "_on_timer_timout".into()),
            (FindingKind::MissingNode, "res://main.tscn:17".into(), "Ghost".into()),
            (FindingKind::MissingNode, "res://main.tscn:18".into(), "Nobody".into()),
        ]
    );
    let missing = &found[0];
    assert!(missing.message.contains("res://player.gd"), "{}", missing.message);
...
"#,
    ...
    assert_eq!(
        summary(&found),
        [
            (FindingKind::MethodArity, "res://main.tscn:15".into(), "_on_one".into()),
            (FindingKind::MethodArity, "res://main.tscn:20".into(), "_on_three".into()),
        ]
    );
    assert!(found[0].message.contains("passes 2 argument(s)"), "{}", found[0].message);
...
fn node_path_in_onready_is_checked_against_every_owning_scene() {
    ...
    assert_eq!(
        summary(&found),
        [
            (FindingKind::MissingNode, "res://ui.gd:3".into(), "Panel/Lable".into()),
            (FindingKind::MissingNode, "res://ui.gd:6".into(), "Nowhere".into()),
        ]
    );
    assert_eq!(found[0].suggestions, ["Label"]);
    for scene in ["res://with_instance.tscn", "res://without_panel.tscn", "res://subclass.tscn", "res://inherited.tscn"] {
        assert!(found[1].message.contains(scene), "{scene} missing from {}", found[1].message);
    }
}
...
"#,
    ...
    assert_eq!(
        summary(&found),
        [
            (FindingKind::MissingNode, "res://hud.gd:5".into(), "%Lives".into()),
            (FindingKind::MissingNode, "res://hud.gd:6".into(), "%Scroe".into()),
        ]
    );
...
fn dynamic_or_unowned_node_paths_produce_no_finding() {
    let owned = scene("Node", Some("res://owned.gd"), "\n[node name=\"Child\" type=\"Node\" parent=\".\"]\n");
...
"#;
    let found = findings(&[
        ("art/icon.png", ""),
        ("art/new_name.png", ""),
        ("art/new_name.png.import", "[remap]\nimporter=\"texture\"\nuid=\"uid://moved\"\n"),
        ("levels/level.tscn", "[gd_scene format=3]\n[node name=\"L\" type=\"Node\"]\n"),
    ...
    assert_eq!(
        summary(&found),
        [
            (FindingKind::MissingResource, "res://main.tscn:3".into(), "res://art/gone.png".into()),
            (FindingKind::MissingResource, "res://main.tscn:6".into(), "res://art/also_gone.png".into()),
            (FindingKind::MissingResource, "res://main.tscn:10".into(), "9_undeclared".into()),
            (FindingKind::MissingResource, "res://main.tscn:12".into(), "res://later.tscn".into()),
            (FindingKind::MissingResource, "res://scripts/loader.gd:2".into(), "res://level.tscn".into()),
            (FindingKind::UnresolvedUid, "res://scripts/loader.gd:6".into(), "uid://unknown".into()),
            (FindingKind::MissingResource, "res://scripts/loader.gd:9".into(), "res://nope.tres".into()),
        ]
    );
...
fn extends_path_and_class_name_references_are_checked() {
    ...
    let found = findings(&[
        ...
        ("base_actor.gd", "class_name Actor\nextends Node\nfunc _on_base_timeout(): pass\n"),
    ...
    assert_eq!(
        summary(&found),
        [
            (FindingKind::DuplicateClassName, "res://dupe_b.gd:2".into(), "Pickup".into()),
            (FindingKind::MissingBaseScript, "res://orphan.gd:1".into(), "res://gone.gd".into()),
        ]
    );
}
...
fn duplicate_uids_and_unparseable_scenes_are_reported() {
    let found = findings(&[
        ...
        ("b.gd", "extends Node\n"),
        ("b.gd.uid", "uid://same\n"),
        ("broken.tscn", "[gd_scene format=3]\n\n[node name=\"A\" type=\"Node\"]\nx = Vector2(1,\n"),
    ]);
    assert_eq!(
        summary(&found),
        [
            (FindingKind::DuplicateUid, "res://b.gd:1".into(), "uid://same".into()),
            (FindingKind::UnparseableScene, "res://broken.tscn:5".into(), "res://broken.tscn".into()),
        ]
    );
}
...
fn findings_are_sorted_by_location_and_deterministic() {
    ...
    let locations: Vec<_> = first.iter().map(|f| (f.at.path.to_string(), f.at.line)).collect();
    assert_eq!(
        locations,
        [("res://a.gd".into(), 2), ("res://a.gd".into(), 3), ("res://m.tscn".into(), 2), ("res://z.gd".into(), 2)]
    );
}
